package com.openclassroom.SafetyNet.utils;

import java.util.Arrays;
import java.util.List;

public class Constants {

    public static final String FIRESTATION_ID = "1";
    public static  final String FIRESTATION_WRONG_ID = "9";
    public static final String WRONG_ADDRESS = "4 Xyz St";
    public static final String ADDRESS = "1 Abc St";
    public static final String MAIL_ADDRESS = "jane.doe@email.com";
    public static final String MAIL_ADDRESS_2 = "johnDoeStation3@email.com";
    public static final String PHONE_NUMBER = "841-874-7784";

    public static final String LAST_NAME = "DoeStation1";
    public static final String FIRST_NAME = "Jane";
    public static final String FIRST_NAME_2 = "John";
    public static final String WRONG_FIRST_NAME = "Wrong";
    public static final String WRONG_LAST_NAME = "Name";
    public static final String BIRTHDATE = "03/06/2020";
    public static final String CITY = "newCity";
    public static final List<String> ALLERGIES = Arrays.asList("newAllergy1", "newAllergy2");
    public static final List<String> MEDICATIONS = Arrays.asList("med1:1mg", "med2:2mg");

}
